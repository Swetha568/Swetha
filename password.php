...
// Check if the username is submitted from the previous page
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['username'])) {
        $_SESSION['username'] = $_POST['username'];
    }
} else {
    // Redirect to the username input page if accessed directly
...
<body>
	...
	<div style=" display: flex; margin-bottom: 54px; margin-top: -5px; margin-left: -3px;">
	<input type="checkbox" id="showPassword" onclick="togglePassword()" style="margin-top: 0px; margin-bottum: 10px; ">
	
            <label for="showPassword">Show Password</label>
	</div>

	
	<span class="forgot" style="margin-top: 20px;">
                <button type="button">Forgot password?</button>
         </span>

	<input type="hidden" name="username" value="<?php echo $_SESSION['username']; ?>">
          <input type="submit" name="sign-in" onclick="validateForm()" value="Sign In">
        </form>
      </div>
